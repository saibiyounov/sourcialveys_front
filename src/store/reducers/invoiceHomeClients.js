...
const reducer = (state = initialState, action) => {
    switch(action.type) {
        case actionTypes.INVOICECLIENT_START: return invoiceStart(state, action)
        case actionTypes.INVOICECLIENT_SUCCESS: return invoiceSuccess(state, action)
        case actionTypes.INVOICECLIENT_FAIL: return invoiceFail(state, action)
        case actionTypes.INVOICECLIENT_SET_PAGE: return invoiceSetPage(state, action)
        case actionTypes.INVOICECLIENT_SET_PAGESIZE: return invoiceSetPageSize(state, action)
        case actionTypes.INVOICECLIENT_SET_FILTERSQUERY: return invoiceSetFiltersQuery(state, action)
        case actionTypes.INVOICECLIENT_SET_SORTQUERY: return invoiceSetSortQuery(state, action)
        case actionTypes.INVOICECLIENT_SET_REVERSESORT: return invoiceSetReverseSort(state, action)
        case actionTypes.INVOICECLIENT_UPDATE_SHOWFILTERS: return invoiceUpdateShowFilters(state, action)
        case actionTypes.INVOICECLIENT_GET_DOCUMENT_FILE: return invoiceGetDocumentFile(state, action)
        case actionTypes.INVOICECLIENT_SELECT_ROW: return invoiceSelectRow(state, action)
        case actionTypes.INVOICECLIENT_SELECT_ALL_ROWS: return invoiceSelectAllRows(state, action)
        case actionTypes.INVOICECLIENT_SET_IMPORT_ERRORS_COUNT: return invoiceSetImportErrorsCount(state, action)
        case actionTypes.INVOICECLIENT_SET_QUICKFILTER: return invoiceSetQuickFilter(state, action)
        default:
            return state
    }
}
